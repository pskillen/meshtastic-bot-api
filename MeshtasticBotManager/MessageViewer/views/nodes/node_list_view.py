...
class NodeListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        latest_position_id = (
            ...
            .exclude(latitude=0, longitude=0, altitude=0)
            .order_by("-logged_time")
            .values("id")[:1]
        )

        # Subquery to get the most recent DeviceMetrics ID for each node
        latest_metrics_id = DeviceMetrics.objects.filter(node=OuterRef("pk")).order_by("-logged_time").values("id")[:1]
            ...
            enriched_nodes.append(
                {
                    ...
                    "last_position_time": (
                        last_position.logged_time.strftime("%Y-%m-%d %H:%M:%S") if last_position else None
                    ),
                    "last_position_ago": (
                        pretty_print_last_heard(last_position.logged_time) if last_position else None
                    ),
