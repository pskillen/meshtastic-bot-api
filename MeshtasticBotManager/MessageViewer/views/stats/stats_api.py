...
from django.db.models import F, ExpressionWrapper, FloatField, Window, Sum
...
class StatsViewSet(viewsets.GenericViewSet):
    def list(self, request):
        ...
        hourly_stats = queryset.annotate(
            hour=TruncHour('time'),
            prev_tx=Window(
                expression=Lag('numPacketsTx'),
                order_by=F('time').asc()
            ),
            prev_rx=Window(
                expression=Lag('numPacketsRx'),
                order_by=F('time').asc()
            ),
            prev_rx_bad=Window(
                expression=Lag('numPacketsRxBad'),
                order_by=F('time').asc()
            ),
            prev_rx_dupe=Window(
                expression=Lag('numRxDupe'),
                order_by=F('time').asc()
            )
        ).values('hour').annotate(
            packets_tx=ExpressionWrapper(
                F('numPacketsTx') - F('prev_tx'),
                output_field=FloatField()
            ),
            packets_rx=ExpressionWrapper(
                F('numPacketsRx') - F('prev_rx'),
                output_field=FloatField()
            ),
            packets_rx_bad=ExpressionWrapper(
                F('numPacketsRxBad') - F('prev_rx_bad'),
                output_field=FloatField()
            ),
            packets_rx_dupe=ExpressionWrapper(
                F('numRxDupe') - F('prev_rx_dupe'),
                output_field=FloatField()
            )
        ).order_by('hour')
